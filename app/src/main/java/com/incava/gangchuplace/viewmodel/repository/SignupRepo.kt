...
class SignupRepo {
    ...
    suspend fun requestSignup(email: String, nickname: String, password: String, passwordConfirm: String): SignupCode {
        return CoroutineScope(Dispatchers.IO).async {
            try {
                ...
                else if (!regexCheck(nickname, "nickname")) return@async SignupCode.NicknameRegexFail
                else if (!regexCheck(password, "password")) return@async SignupCode.PasswordRegexFail
